  ...
  ({ place, onClose, onSavePlace, onReserveTable, onShowOnMap }, ref) => {
    ...
    const mockReviews: ReviewData[] = [
      {
        ...
        author: 'Maria Silva',
        rating: 5,
        comment: 'Hambúrguer incrível! O molho especial é sensacional e a carne no ponto perfeito. Ambiente descontraído e atendimento nota 10.',
        timeAgo: 'há 2 dias'
      },
      {
        id: '2',
        author: 'João Santos',
        rating: 4,
        comment: 'Muito bom! As batatas são crocantes e o hambúrguer bem saboroso. Só achei um pouco demorado para ficar pronto.',
        timeAgo: 'há 1 semana'
      },
      {
        id: '3',
    ...
    const showBottomSheet = () => {
      ...
      const bottomSheetContent = (
        ...
        >
          <BottomSheetScrollView style={{ flex: 1, backgroundColor: 'white' }}>            
              {/* Header Image */}
              {renderHeaderImage()}
    ...
    const renderPlaceInfo = () => (
      <View className="px-4 py-4">
        <Text className="text-2xl font-bold text-gray-900 mb-2">
          {place?.googleData.name || 'Local sem nome'}
        </Text>
          ...
          <View className="flex-row items-center">
            ...
            <View className="flex-1">
              <Text className="text-gray-900 font-medium">
                {getAddressString(place?.googleData.address)}
              </Text>
        ...
        <View className="flex-row items-center">
          ...
          <View className="bg-primary-500 rounded-2xl p-4 mr-4 items-center justify-center w-20 h-20">
            <Text className="text-white text-2xl font-bold">
              {place?.googleData.rating?.toFixed(1) || '4.6'}
            </Text>
    ...
    const renderPinubiReview = () => (
      <View className="px-4 pb-6">
        ...
        <Text className="text-gray-700 text-base leading-relaxed mb-4">
          O {place?.googleData.name || 'local'} se destaca por seus hambúrgueres artesanais criativos, 
          ambiente descontraído e amigável, e excelente atendimento. No entanto, pode 
          haver algumas limitações quando se trata de acomodar grupos maiores durante 
          horários de pico.
        </Text>
    ...
    const renderCommunityReviews = () => (
      <View className="px-4 pb-6">
        ...
        <Text className="text-gray-600 text-sm mb-4">
          {mockReviews.length} avaliações públicas
        </Text>

        <View className="space-y-4">
          {mockReviews.map((review) => (
            <View key={review.id} className="border-b border-gray-100 pb-4 last:border-b-0">
    ...
    const handleReserveTable = () => {
      if (place && onReserveTable) {
        ...
        Alert.alert(
          'Reservar Mesa',
          'Funcionalidade de reserva em desenvolvimento.',
          [{ text: 'Ok' }]
        );
      }
    };
